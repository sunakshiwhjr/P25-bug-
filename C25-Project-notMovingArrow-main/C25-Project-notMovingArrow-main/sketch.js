...
//he has done as per class activity
function keyPressed() {

  if(keyCode === RIGHT_ARROW){
    console.log("Pressed")
    // create an arrow object and add into an array ; set its angle same as angle of playerArcher
    var arrow = new PlayerArrow(playerArcher.x, playerArcher.y,80,80);
...
function showArrows(arrows,index) {
  //arrows.display();
  arrows.display();
  if(arrows.body.position.x >=width || arrows.body.position.y >= height -100)
  {
    //  World.remove(borld, arrows.body);
